...
import seanowenhayes.TelloVR.BuildConfig
...
// default activity
class TelloVrActivity : AppSystemActivity() {
    ...
    override fun onSceneReady() {
        ...
        componentManager.registerComponent<SpatializedAudioPanel>(SpatializedAudioPanel)
    ...
    override fun onVRReady() {
        ...
        if (!isFirstReadyDone) {
            ...
            mrPanelPose =
                Entity(R.integer.spatialized_video_panel).getComponent<Transform>().transform
            setMrMode(scene.isSystemPassthroughEnabled())
                    ...
                    object : SeekBar.OnSeekBarChangeListener {
                        override fun onProgressChanged(
                            ...
                            if (fromUser) {
                                ...
                                Entity(R.integer.spatialized_video_panel).setComponent(
                                    Scale(
                                        newScale
                                    )
                                )
        ...
        return PanelRegistration(R.layout.controls) {
            ...
            panel {
                ...
                backButton.setOnClickListener {
                    setUri?.let {
                        MoviePanel.moviesViewModel.previousVideo(
                            it
                        )
                    }
                }
                ...
                forwardButton.setOnClickListener {
                    setUri?.let {
                        MoviePanel.moviesViewModel.nextVideo(
                            it
                        )
                    }
                }
        ...
        if (isMrMode) {
            ...
            Entity(R.integer.controls_id)
                .setComponent(
                    Transform(
                        Pose(
                            Vector3(0.0f, -0.43f, -0.15f),
                            Quaternion(20f, 0f, 0f)
                        )
                    )
                )
        } else {
